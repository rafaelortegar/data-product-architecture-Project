...
from metadataModeling import metadataModeling
...
class predictionMetro(luigi.Task):
    ...
    def requires(self):
        return metadataModeling(bucket=self.bucket, date=self.date) # , metadataCleaned(bucket = self.bucket, date=  self.date)
    ...
    def run(self):
        ...
        df = psql.read_sql('SELECT * FROM modeling.metadata ORDER BY accuracy_promedio DESC LIMIT 1 ;', connection)
        cursor.close()
        
        modelobuscado = df['nombre_modelo'][0]
