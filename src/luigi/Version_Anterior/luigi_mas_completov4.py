...
class metadataExtract(luigi.Task):
    ...
    def requires(self):
        return extractToJson(bucket = self.bucket, date = self.date)                       # (bucket = self.bucket, date = self.date)
...
#        client.get('Reservations')[0].get('Instances')[0].get('KeyName')
    ...
    def output(self):
        output_path = "s3://dpaprojs3/metadataExtract_task_02_02/metro_{}.csv". \
...
class copyToPostgres(luigi.Task):
    ...
    def requires(self):
        ...
        #extractToJson(bucket=self.bucket, date=self.date), metadataExtract(bucket=self.bucket, date=self.date)
...
#        client.get('Reservations')[0].get('Instances')[0].get('KeyName')
        ...
        print("#####...")
        print("######...")
        print("Carga de metadatos de Feature Engineering completada! :)")
