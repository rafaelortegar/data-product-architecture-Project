...
sudo echo "host all  all    0.0.0.0/0  md5" >> /etc/docker/$POSTGRESQL_VERSION/main/pg_hba.conf
sudo echo "local all all md5" >> /etc/docker/$POSTGRESQL_VERSION/main/pg_hba.conf
sudo echo "listen_addresses='*'" >> /etc/docker/$POSTGRESQL_VERSION/main/postgresql.conf
psql --command "ALTER USER metro WITH PASSWORD 'postgres';"
